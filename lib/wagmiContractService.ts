...
const CONTRACT_ADDRESS = '0x61d11C622Bd98A71aD9361833379A2066Ad29CCa' as const;
...
/**
 * Hook to check if the connected address is the contract owner
 */
...
/**
 * Hook to get the available house profit (user losses)
 */
export function useHouseProfit() {
  const { data, isLoading, error, refetch } = useReadContract({
    address: CONTRACT_ADDRESS,
    abi: CONTRACT_ABI,
    functionName: 'getHouseProfit',
  });

  return {
    houseProfit: data ? formatEther(data) : "0",
    houseProfitWei: data || BigInt(0),
    isLoading,
    error,
    refetch
  };
...
/**
 * Hook for admin to withdraw house profit
 */
export function useWithdrawHouseProfit() {
  const { writeContract, isPending, error, data: hash } = useWriteContract();
  const { isLoading: isConfirming, isSuccess: isConfirmed } = useWaitForTransactionReceipt({
    hash,
  });

  const withdrawHouseProfit = async (amount: string) => {
    try {
      console.log('💰 Withdrawing house profit:', amount);
      ...
      await writeContract({
        address: CONTRACT_ADDRESS,
        abi: CONTRACT_ABI,
        functionName: 'withdrawHouseProfit',
        args: [amountWei],
      });
      
      return hash;
    } catch (error: any) {
      console.error('❌ Error withdrawing house profit:', error);
      throw new Error(error.message || 'Failed to withdraw house profit');
    }
  };
