...
export default function CrashGame({ compact = false }: CrashGameProps) {
  ...
  const betAmountRef = useRef<string>("0.00")
  const gameRoundRef = useRef<GameRound>({ multiplier: 1.0, crashed: false, isRunning: false })
  ...
  const startNewRound = () => {
    const newGameRound = { multiplier: 1.0, crashed: false, isRunning: true }
    gameRoundRef.current = newGameRound
    setGameRound(newGameRound)
    setCurrentMultiplier(1.0)
    const initialChartData = [{ x: 0, y: 1.0 }]
    ...
    gameIntervalRef.current = setInterval(() => {
      ...
      if (hasBetRef.current) {
        const bet = Number.parseFloat(betAmountRef.current) || 0
      ...
      setChartData((prev) => {
        ...
        if (!lastPoint || elapsed - lastPoint.x > 0.1) {
          const newData = [...prev.slice(-100), { x: elapsed, y: multiplier }]
      ...
      if (gameRoundRef.current.isRunning && !gameRoundRef.current.crashed) {
        const rect = canvasRef.current?.getBoundingClientRect()
        const width = rect?.width || 0
        const height = rect?.height || 0
        const maxTime = Math.max(...chartDataRef.current.map((d) => d.x), 10)
