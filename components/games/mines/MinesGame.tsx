...
export default function MinesGame({ compact = false, onBack }: MinesGameProps) {
  ...
  const { startGame: startGameContract, resolveGame: resolveGameContract, getUserStats, isCorrectNetwork } = useContract()
  ...
  // Resolve game automatically via API (no user signature required)
  const resolveGame = async (didWin: boolean, finalMultiplier: number) => {
    ...
    try {
      console.log(`🚀 Auto-resolving game: ${gameId}, Win: ${didWin}, Multiplier: ${finalMultiplier}`);
  ...
  const handleBet = async () => {
    ...
    if (isPlaying) {
      ...
      try {
        ...
        const result = await startGameContract(newGameId, "mines", betInWei)
        ...
        // @ts-ignore - best effort error message
          ...
          errorMsg = "Insufficient balance. Please add more ETH to your wallet."
        // @ts-ignore - best effort error message
          ...
          <div className="space-y-3">
            ...
            {isConnected && !isCorrectNetwork && (
              <div className="bg-red-600/10 border border-red-500/20 rounded-4xl p-3 text-center">
                <p className="text-red-400 text-sm font-medium">
                  ⚠️ Please switch to Sepolia network to play
                </p>
                <p className="text-red-300 text-xs mt-1">
                  Games only work on Sepolia testnet
                </p>
              </div>
            )}
            ...
            <div>
              ...
              <div className="flex items-center gap-2 rounded-xl border border-border bg-background/70 px-3 py-2 text-sm">
                <span className="text-foreground/60">{totalProfit} ETH</span>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-background/30 px-3 py-2 text-xs text-foreground/70">
              ...
              {gameOver && totalProfit !== "0.00" && `You cashed out with ${totalProfit} ETH at ${multiplier.toFixed(2)}×.`}
              ...
              <div className="text-center space-y-3">
                {popup.type === "mine" ? (
                  ...
                  <>
                    ...
                    <p className="text-foreground/70 text-base">You cashed out with {totalProfit} ETH profit at {multiplier.toFixed(2)}× multiplier!</p>
                  </>
                )}
