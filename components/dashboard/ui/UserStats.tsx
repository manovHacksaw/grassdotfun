...
import { useUserStats } from "@/context/hooks"
...
export default function UserStats() {
  ...
  const { withdraw: contractWithdraw } = useContract()
  const { data: contractUserStats, isLoading: isContractLoading, error: contractError } = useUserStats(accountId as `0x${string}`)
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (accountId) {
        ...
        // Note: UserStats will be fetched via the useUserStats hook
        console.log("📊 User stats will be fetched via hook")
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      console.log(`💸 Withdrawing ${withdrawableAmount.toFixed(4)} ETH`)
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              ...
              <p className="text-yellow-300 text-sm mb-3">
                You have <span className="font-bold text-yellow-200">{parseFloat(userStats.withdrawableBalance).toFixed(4)} ETH</span> ready to withdraw
              </p>
              <Button
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    💰 Withdraw {parseFloat(userStats.withdrawableBalance).toFixed(4)} ETH
                  </>
                )}
              </Button>
            </div>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={chartData}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number) => [`${value.toFixed(4)} ETH`, 'Profit/Loss']}
                />
                <Area
                  type="monotone"
                  dataKey="profit"
              ...
              <tbody>
                {gameStats.map((game, index) => (
                  <tr key={index} className="border-b border-border/50">
                    ...
                    <td className="py-3 px-4 text-green-400 font-medium">{parseFloat(game.totalWon.toString()).toFixed(4)} ETH</td>
