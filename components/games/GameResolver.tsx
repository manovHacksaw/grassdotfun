...
// ContractService and ResolverService are deprecated - using wagmi hooks directly
...
export default function GameResolver({ gameId: initialGameId, onResolved, compact = false }: GameResolverProps) {
  ...
  // Services are deprecated - using wagmi hooks directly
  useEffect(() => {
    if (address) {
  ...
  const checkIfResolver = async () => {
    ...
    try {
      // For now, assume the deployer is the resolver
      // In production, you would check the contract's resolver address
      const resolverAccount = "0x2D4575003f6017950C2f7a10aFb17bf2fBb648d2" // From deployment info
      setIsResolver(address.toLowerCase() === resolverAccount.toLowerCase())
    } catch (error) {
      ...
      {gameDetails && (
        <div className="bg-blue-600/20 border border-blue-500/30 rounded-xl p-4 mb-4">
          ...
          <div className="space-y-1 text-sm text-blue-300">
            ...
            <p>Amount: {(parseFloat(gameDetails.amount) / 1e24).toFixed(4)} U2U</p>
