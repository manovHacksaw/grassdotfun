...
export function CoinFlip({ compact = false }: CoinFlipProps) {
  const { accountId, isConnected, balance, isBalanceLoading } = useWallet()
  ...
  // Auto-resolve game when it ends
  const resolveGame = async (didWin: boolean, finalMultiplier: number) => {
    ...
    try {
      console.log(`🚀 Auto-resolving coinflip game: ${gameId}, Win: ${didWin}, Multiplier: ${finalMultiplier}`);
      
      // Convert multiplier to percentage (e.g., 2.5x = 250%)
      const multiplierPercent = Math.round(finalMultiplier * 100);
      
      // Use the automatic resolution service (no user signature required)
      const result = await gameResolutionService.resolveGame({
        gameId,
        didWin,
        multiplierPercent
      });
      ...
      if (result.success) {
        console.log(`✅ Coinflip game auto-resolved successfully! Hash: ${result.hash}`);
      } else {
        console.error(`❌ Failed to auto-resolve coinflip game: ${result.error}`);
      }
    } catch (error: any) {
      console.error("❌ Error auto-resolving coinflip game:", error);
    }
  };
  const [selectedSide, setSelectedSide] = React.useState<Side | null>(null)
  const [betAmount, setBetAmount] = React.useState<string>("0.001")
  ...
  // Contract service is now handled by the contract provider
  
  // Auto-resolve game when status changes to won or lost
  React.useEffect(() => {
    if (status === "won" && gameId && currentMultiplier > 0) {
      resolveGame(true, currentMultiplier);
    } else if (status === "lost" && gameId) {
      resolveGame(false, 0);
    }
  }, [status, gameId, currentMultiplier]);
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 rounded-xl border border-border bg-background/70 px-3 py-2 text-sm">
                <span className="text-foreground/60">{potentialPayout.toFixed(4)} ETH</span>
              </div>
            </div>
              ...
              <div className="text-center space-y-3">
                {popup.type === "lose" ? (
                  ...
                  <>
                    ...
                    <p className="text-foreground/70 text-base">
                      Streak: {currentStreak} | Multiplier: {currentMultiplier.toFixed(2)}× | Won: {totalPayout.toFixed(4)} ETH
                    </p>
