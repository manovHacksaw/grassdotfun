...
// 🎲 TERMINAL 3 - Optimized for 1.8 CELO budget (runs simultaneously with Terminal 2)
const CONTRACT_ADDRESS = process.env.CONTRACT_ADDRESS || "0x61d11C622Bd98A71aD9361833379A2066Ad29CCa";
const RESOLVER_API_URL = process.env.RESOLVER_API_URL || "http://grassdotfun.vercel.app/api/resolve-game-production";
...
const WALLETS_FILE = path.join(__dirname, "generated-wallets-terminal3.json");
...
function generateWallets(count) {
  console.log(`\n🔑 [TERMINAL 3] Creating ${count} random wallets...\n`);
  const wallets = [];
  ...
  console.log(`\n✅ Wallets saved to generated-wallets-terminal3.json`);
  return wallets;
}
...
async function fundWallets(wallets, amountPerWallet) {
  console.log(
    `\n💰 [TERMINAL 3] Checking and funding wallets...\n`
  );
...
function calculateGameOutcome(gameType, betAmount) {
  ...
  // Final safety check: ensure multiplier never exceeds max and win never exceeds 0.1 CELO
  multiplier = Math.min(multiplier, effectiveMaxMultiplier);
    ...
    try {
      ...
      try {
        ...
        if (resolveResponse.data.success) {
          ...
          console.log(
            `✅ [T3] Game ${gameNumber}/${totalGames} (${progress}%) | ` +
        ...
        console.error(
          `⚠️  [T3] API resolution failed for game ${gameNumber}:`,
          apiError.message
        );
      ...
      if (attempt < retries) {
        console.error(
          `⚠️  [T3] Game ${gameNumber} attempt ${attempt} failed, retrying...`
        );
        console.error(`   Error: ${errorMsg.slice(0, 100)}`);
        await sleep(5000);
      } else {
        console.error(
          `❌ [T3] Game ${gameNumber} failed after ${retries} attempts:`
        );
...
async function main() {
  console.log("\n" + "=".repeat(70));
  console.log("🎲 TERMINAL 3 - RANDOMIZED GAME TRANSACTION GENERATOR");
  ...
  if (!existingWalletsFunded) {
    ...
    console.log(`   Minimum required per wallet: ${(parseFloat(BET_AMOUNT) * 5 + 0.01).toFixed(4)} CELO`);
    console.log(`   Target balance per wallet: ${FUNDING_PER_WALLET} CELO`);
  }

  console.log(`\n📋 Configuration:`);
  console.log(`   Terminal: 3 (Third set - runs simultaneously with Terminal 2)`);
  ...
  console.log(`\n⏳ [TERMINAL 3] Starting randomized game execution...\n`);
  await sleep(3000);
  ...
  console.log("\n" + "=".repeat(70));
  console.log("🎉 [TERMINAL 3] COMPLETE!");
  console.log("=".repeat(70));
  console.log(`\n📊 Final Summary:`);
  ...
  console.log(
    `   npx hardhat run scripts/collect-funds-terminal3.cjs --network celo\n`
  );
}
